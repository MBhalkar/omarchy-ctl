...
// Themed single-select dropdown, vendored from the shell kit (Ui/Dropdown.qml)
// with an `openUpward` option so the popup can open above the trigger when the
// trigger sits near the bottom edge of a panel.
...
Item {
  ...
  function open() { if (root.options.length > 1) popup.open() }
  function close() { popup.close() }
  function toggle() {
    if (root.options.length < 2) return
    popup.opened ? popup.close() : popup.open()
  }

  readonly property int popupContentHeight: {
    var n = Math.max(1, root.options.length)
    var raw = n * root.popupRowHeight + Math.max(0, n - 1) * Style.spacing.labelGap + Style.spacing.xxs
    return Math.min(raw, root.popupRowHeight * 8 + 7 * Style.spacing.labelGap + Style.spacing.xxs)
  }
    ...
    BorderSurface {
      ...
      Keys.onPressed: function(event) {
        if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter
          ...
          if (root.options.length > 1) popup.opened ? popup.close() : popup.open()
          event.accepted = true
      ...
      Popup {
        ...
        x: 0
        y: root.openUpward
          ? -(root.popupContentHeight + Style.spacing.xxs)
          : trigger.height + Style.spacing.xxs
        width: trigger.width
        implicitHeight: root.popupContentHeight
